...
class ZooAPIClient {
  constructor(queryId, accountIndex, baseURL) {
    this.headers = {
      Accept: "*/*",
  ...
  log(msg, type = "info") {
    ...
    switch (type) {
      case "success":
        console.log(`[${timestamp}]${accountPrefix} [✓] ${msg}`.green);
        break;
      case "custom":
        console.log(`[${timestamp}]${accountPrefix} [*] ${msg}`.magenta);
        break;
      case "error":
        console.log(`[${timestamp}]${accountPrefix} [✗] ${msg}`.red);
        break;
      case "warning":
        console.log(`[${timestamp}]${accountPrefix} [!] ${msg}`.yellow);
        break;
      default:
        console.log(`[${timestamp}]${accountPrefix} [ℹ] ${msg}`.blue);
    }
  }
  ...
  async runAccount() {
    ...
    try {
      ...
      console.log(`=========Account ${i + 1}| ${firstName + " " + lastName} | Starting in ${timesleep} seconds.....`.green);
      this.set_headers();
...
async function main() {
  ...
  queryIds.map((val, i) => new ZooAPIClient(val, i, hasIDAPI).createUserAgent());

  await sleep(1);
  while (true) {
    for (let currentIndex = 0; currentIndex < queryIds.length; currentIndex++) {
      const client = new ZooAPIClient(queryIds[currentIndex], currentIndex, hasIDAPI);
